...
重命名行为由 `config/rename/profiles.yaml` 管理。宿主必须传入 `profile=<id>`；除不参与配置的 `noCache` 外，其他 URL 参数一律拒绝。
...
地区识别使用确定的信号优先级，并将无法识别的节点归入 `OTHER`。rename 使用同一 metadata 生成显示名称；override 直接使用 metadata 分类，不要求节点先经过 rename。
...
```text
Sub-Store proxies + $arguments.profile
→ adapter 解析命名 profile
→ node-domain 提取地区、标签、倍率和元数据
→ rename runtime 生成稳定名称
→ 返回新代理数组与结构化诊断
```
