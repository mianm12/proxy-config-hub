var __proxyConfigHub = (() => {
  ...
  function resolveProviderRuleTarget(providerId, targetGroupId, groupDefinitions2) {
  ...
  function assembleRuleSet(groupDefinitions2, ruleProviders2, inlineRules) {
    ...
    for (const [providerId, providerDefinition] of Object.entries(ruleProviders2)) {
      ...
      const targetName = resolveProviderRuleTarget(providerId, targetGroupId, groupDefinitions2);
      const provider = {};
      ...
      rules.push(
        providerDefinition["no-resolve"] ? `RULE-SET,${providerId},${targetName},no-resolve` : `RULE-SET,${providerId},${targetName}`
      );
    }
